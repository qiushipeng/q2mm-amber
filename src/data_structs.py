...
class Datum(object):
    ...
    def __init__(
        ...
        self.atm_4 = atm_4
        self.ff_row = ff_row
        # Bond separation of the two atoms of a calculated Hessian element,
        # from the molecule's topology (Structure.bonds_apart): 0 same atom,
        # 1 bonded, 2 angle endpoints, 3 dihedral endpoints, 4 further apart.
        # score.import_weights turns it into the per-element weight and FXATM
        # drops the sep > 3 elements of fixed atoms. None otherwise.
        self.sep = sep
...
class Structure(object):
    ...
    def pair_separations(self) -> dict:
        """Bond separation of every atom pair this structure's bonds, angles
        and torsions connect: {frozenset({a, b}): 1 (bonded), 2 (angle
        endpoints) or 3 (torsion endpoints)}. In a ring a pair can be several
        of these at once; the shortest wins. Pairs further apart are absent.

        Returns:
            dict: {frozenset of two 1-based atom indices: bonds apart}
        """
        separations = {}
        for separation, dofs in ((1, self.bonds), (2, self.angles), (3, self.torsions)):
            for dof in dofs:
                separations.setdefault(
                    frozenset((dof.atom_nums[0], dof.atom_nums[-1])), separation)
        return separations

    def bonds_apart(self, atom_a: int, atom_b: int, separations: dict = None) -> int:
        """Bond separation of two atoms (1-based indices): 0 for the same atom,
        1, 2 or 3 as in pair_separations, and 4 for anything further apart.

        Args:
            atom_a (int): first atom index
            atom_b (int): second atom index
            separations (dict, optional): the dict from pair_separations, to
                reuse when asking about many pairs. Built here when not given.

        Returns:
            int: bonds apart, 0 to 4
        """
        if atom_a == atom_b:
            return 0
        if separations is None:
            separations = self.pair_separations()
        return separations.get(frozenset((atom_a, atom_b)), 4)
...
def datums_from_hessian(hessian: np.ndarray, src: str, structure: Structure = None) -> List[Datum]:
    """One Datum (typ 'h') per lower-triangular element of a 3N x 3N
    mass-weighted Hessian; idx_1/idx_2 are the 1-based cartesian degrees of
    freedom. The Hessian is symmetric, so the upper triangle would only
    duplicate data points.

    With a Structure (the calculated side), every element also carries its
    two atoms and their bond separation (Structure.bonds_apart), which
    scoring turns into the per-element weight. Without one (the reference
    side) the elements get the uniform Hessian weight at score time.

    Args:
        hessian (np.ndarray): 3N x 3N symmetric matrix
        src (str): source file name; its stem labels the data
        structure (Structure, optional): the molecule, for the atom-pair separations

    Returns:
        List[Datum]: 3N(3N+1)/2 data points
    """
    tri_i, tri_j = np.tril_indices_from(hessian)
    separations = structure.pair_separations() if structure is not None else None
    data = []
    for i, j in zip(tri_i, tri_j):
        datum = Datum(val=float(hessian[i, j]), typ="h", src_1=src,
                      idx_1=int(i + 1), idx_2=int(j + 1))
        if structure is not None:
            # atoms are idx // 3 + 1 in 1-based numbering
            datum.atm_1 = int(i // 3 + 1)
            datum.atm_2 = int(j // 3 + 1)
            datum.sep = structure.bonds_apart(datum.atm_1, datum.atm_2, separations)
        data.append(datum)
    return data


def datums_from_geometry(structures: List[Structure], kind: str, src: str) -> List[Datum]:
    """Datum objects of one kind ('b' bonds, 'a' angles, 't' torsions) for
    every structure, in order. The 1-based structure index is stamped into
    each Datum (idx_1) so torsion labels satisfy constants.RE_T_LBL
    ("t_<name>_<idx>_<a-b-c-d>"); without it score.trim_data's
    RE_T_LBL.split(...)[1] raises IndexError.

    Args:
        structures (List[Structure]): the measured structures
        kind (str): 'b', 'a' or 't'
        src (str): source file name; its stem labels the data

    Returns:
        List[Datum]: one data point per bond/angle/torsion
    """
    attribute = {"b": "bonds", "a": "angles", "t": "torsions"}[kind]
    data = []
    for i, structure in enumerate(structures, 1):
        for dof in getattr(structure, attribute):
            data.append(dof.as_data(src_1=src, idx_1=i))
    return data


def datums_from_eigenmatrix(matrix: np.ndarray, src: str) -> List[Datum]:
    """One Datum (typ 'eig') per lower-triangular element of an
    n_modes x n_modes eigenmatrix; idx_1/idx_2 are the 1-based mode numbers,
    mode 1 being the lowest (the transition-state mode). The reference side
    is diag(QM eigenvalues); the calculated side is the force-field Hessian
    projected on the QM modes (math_util.project_hessian). Weights come from
    score.import_weights: eig_i, eig_d_low/eig_d_high, eig_o.

    Args:
        matrix (np.ndarray): n_modes x n_modes symmetric matrix
        src (str): source file name; its stem labels the data

    Returns:
        List[Datum]: n_modes (n_modes + 1) / 2 data points
    """
    tri_i, tri_j = np.tril_indices_from(matrix)
    return [Datum(val=float(matrix[i, j]), typ="eig", src_1=src,
                  idx_1=int(i + 1), idx_2=int(j + 1))
            for i, j in zip(tri_i, tri_j)]


def datum_from_energy(value: float, idx: int, src: str, typ: str = "e") -> Datum:
    """A single energy data point; idx is the structure (conformer) number."""
    return Datum(val=float(value), typ=typ, src_1=src, idx_1=int(idx), idx_2=1)


# endregion Datum builders
...
class AmberFF(FF):
    """
    An Amber force field: the fitted parameters (ParAMBER, one per frcmod
    value) plus the lines of the frcmod they were read from, which the
    parameters are rendered back onto when the force field is written.

    Pure data. utilities.AmberUtilities.read_frcmod builds one from a file
    and write_frcmod (or a utilities.Frcmod) writes it out; nothing here
    touches a file.
    """
    ...
    @property
    def lines(self):
        """Template lines of the frcmod this force field was read from; None
        for a force field built from parameters alone."""
        return self._lines
